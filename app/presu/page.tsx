...
export default function Presupuestador() {
  ...
  const generarPDF = async () => {
    ...
    setCargando(true);
    try {
      // Configuramos el escáner para que ignore las protecciones de imagen (useCORS)
      ...
      alert("Error del sistema PDF: " + error.message);
    }
    setCargando(false);
  };
    ...
    <div className="min-h-screen bg-zinc-100 p-4 md:p-8 font-sans flex flex-col lg:flex-row gap-8">
      ...
      <div className="w-full lg:w-1/3 bg-white p-6 rounded-3xl shadow-lg border border-zinc-200 flex flex-col gap-6 h-fit">
        ...
        <div className="space-y-3">
          ...
          <input type="text" placeholder="Institución / Colegio" className="w-full p-2 text-sm border rounded-lg bg-zinc-50 outline-none focus:border-blue-500" 
            value={cliente.institucion} onChange={e => setCliente({...cliente, institucion: e.target.value})} />
          <input type="text" placeholder="Nombre del Contacto" className="w-full p-2 text-sm border rounded-lg bg-zinc-50 outline-none focus:border-blue-500" 
            value={cliente.contacto} onChange={e => setCliente({...cliente, contacto: e.target.value})} />
          <div className="flex gap-2">
            <input type="text" placeholder="RUC" className="w-1/2 p-2 text-sm border rounded-lg bg-zinc-50 outline-none focus:border-blue-500" 
              value={cliente.ruc} onChange={e => setCliente({...cliente, ruc: e.target.value})} />
            <input type="text" placeholder="Teléfono" className="w-1/2 p-2 text-sm border rounded-lg bg-zinc-50 outline-none focus:border-blue-500" 
        ...
        >
          {cargando ? "Procesando Documento..." : "📥 Descargar PDF Oficial"}
        </button>
      </div>
      ...
      <div className="flex-1 flex justify-center overflow-x-auto pb-10">
        <div 
          ref={pdfRef} 
          className="bg-white shadow-2xl shrink-0 relative flex flex-col"
          style={{ width: '210mm', minHeight: '297mm', padding: '15mm' }} 
        >
          {/* Header del Presupuesto */}
          <div className="flex justify-between items-start border-b-2 border-blue-900 pb-6 mb-6">
            <div className="flex items-center gap-4">
              {/* 🔥 FIX: Imagen estándar HTML que el PDF sí puede leer */}
              <div className="w-20 h-20 bg-zinc-100 rounded-lg flex items-center justify-center overflow-hidden">
                <img src="/logo.png" alt="Metal Mad Logo" className="w-full h-full object-contain" crossOrigin="anonymous" />
              </div>
              <div>
              ...
              {items.map((item, index) => (
                <tr key={index} className="border-b border-zinc-100">
                  <td className="p-2 text-center">
                    <input type="number" value={item.cantidad} onChange={e => actualizarCantidad(index, Number(e.target.value))} className="w-12 text-center border rounded bg-transparent font-bold outline-none" />
                  </td>
                  <td className="p-2 font-medium text-zinc-800">{item.nombre}</td>
                  <td className="p-2 text-right">
                    <input type="number" value={item.precioUnitario} onChange={e => actualizarPrecio(index, Number(e.target.value))} className="w-24 text-right border-b border-dashed border-zinc-300 bg-transparent outline-none" />
                  </td>
